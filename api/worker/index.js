...
// not used
function sort_by_index(arr, index) {
...
async function get_data_freetext(params) {
    ...
    let arr = s.split(' '); // could allow more parsing (e.g. with +, -, "")
...
async function get_data_orgnr(orgnr) {
    const val = await EMPLOYER.get(orgnr, { type: "json" })
...
async function handleRequest(request) {
    ...
    if (Object.keys(params).length < 3) {
        ...
        let data = []
        if(is_orgnr)
            data = await get_data_orgnr(Object.values(params)[0])
        else if(is_freetext)
            data = await get_data_freetext(params)
        else
            data = await get_data_structured(params);

        if(data == null || data.length == 0)
            return new Response(JSON.stringify({ "error": "request not valid" }), { headers: all_headers });
        
        // intersect if multiple returns
        data = data.length == 1? data[0]: intersect(data);

        result = is_orgnr? data: create_output(data, page, per_page);
    }
    else
        result = { 'error': 'too many parameters'}

    result['response_time_kv'] = Date.now() - requestStartTime_kv
...
// Can be used if deployed as a cloudflare function
